...
watch(() => state.slideAttrTitle, newValue => {
  if (newValue) {
    setTimeout(() => {
...
const bindEvents = () => {
  ...
  document.addEventListener(move, (e: any) => {
    ...
    // 10 到 43是可滑动区域
    if (moveX < 10 || moveX + 43 >= props.w) return false;
    ...
    <transition>
      <div class="slideSty animated bounce">
        ...
        >
          ...
          <img ref="block" :width="state.sliderImgWidth" :height="state.sliderImgHeight" class="slide-verify-block" :src="state.sliderImg" />
          <!-- container -->
          <div
          ...
          >
            ...
            <span class="slide-verify-slider-text">{{ t('captcha.sliderCaptcha') }}</span>
          </div>
        </div>
        <div class="iconBtn">
